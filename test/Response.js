...
describe("Response", () => {
  ...
  it("should allow array of attachment in res.attachment()", () => {
    const res = new Response(token, dataStoreMock, requestMock);
    res.attachment("hello", [{ a: "b" }, { c: "d" }, { e: "f" }]);
  ...
  it("allow channel name (not id) to be passed in optTargets", () => {
    ...
    dataStoreMock.getChannelOrGroupByName
      .withArgs("#general")
      .returns(channelMock);
  ...
  it("should not handle unknown task type", done => {
    ...
    res.on("task_finished", (task, data) => {
      ...
      data.should.be.deep.equal({
        message: `Unknown task type ${unknownTask.type}`
      });
      done();
    });
  ...
  it("should be able to send file", done => {
    ...
    nock("https://slack.com")
      ...
      .reply(200, (uri, body) => {
        (
          body.indexOf('Content-Disposition: form-data; name="file"') === -1
        ).should.be.equal(true);
        (
          body.indexOf('Content-Disposition: form-data; name="content"') > -1
        ).should.be.equal(true);
        return {
          ok: true,
          id: "F306471"
  ...
  it("should allow use file opts when using readable stream", done => {
    ...
    res
      .upload("Response.js", fs.createReadStream("test/fixtures/img.png"))
      .send();
  });
